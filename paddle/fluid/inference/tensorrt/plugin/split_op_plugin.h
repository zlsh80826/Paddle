...
class SplitPlugin : public PluginTensorRT {
 public:
  ...
  SplitPlugin* clone() const TRT_NOEXCEPT override {
    auto* ptr = new SplitPlugin(axis_, output_length_, with_fp16_);
    ptr->shareData(this);
    return ptr;
  }

  const char* getPluginType() const TRT_NOEXCEPT override { return "split_plugin"; }
  int getNbOutputs() const TRT_NOEXCEPT override { return output_length_.size(); }
...
class SplitPluginV2Creator : public nvinfer1::IPluginCreator {
 public:
  ...
  const char* getPluginName() const TRT_NOEXCEPT override { return "split_plugin"; }
